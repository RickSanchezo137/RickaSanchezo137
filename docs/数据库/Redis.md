...
缓存由于某些原因（比如 宕机、cache服务挂了、大量数据同时过期或者不响应）整体crash掉了，导致大量请求到达后端数据库，从而导致数据库崩溃，整个系统崩溃，发生灾难
...
1. 使用集群缓存，保证缓存服务高可用
2. 随机化过期：在指定的缓存数据的过期时间上，再增添一个随机的时间，防止同一时间大量数据过期现象发生
3. 如果发现缓存出问题，及时对相应的请求进行熔断、服务降级

### 缓存穿透

#### 场景

去缓存中查询数据，有一定几率查询失败，这时就要数据库查询，如果直接去查询一条缓存中和数据库中都不存在的数据，那么这个查询就会百分之百查询到数据库上去

如果有人恶意拿一个不存在的数据去查询，会产生大量请求，这些请求最终会打到数据库中，数据库可能因为承受不住压力而宕机

#### 解决方案

1. 缓存空值。每次查询为空，就缓存该请求或键，下次直接避免打到数据库

   > 这个方法存在问题：
   >
   > 1. 缓存空值需要更多的内存空间，我们可以设置一个比较短的过期时间，过了这个时间，自动剔除这个空值的缓存
   > 2. 空值应该与正常值分开存放，否则当空间不足的时候，缓存系统可能有优先剔除正常值，再剔除空值，这个漏洞可能会被攻击
   > 3. 如果某个key在缓存中记录为空值，过了一段时间，数据库中添加了这个key，那此时需要利用某种方式来清除这个空值。如果使用的是redis缓存，更新数据后直接在redis中清除即可

2. 布隆过滤器。每次查询后将查询结果不为空的请求或键添加到布隆过滤器，这样，所有有效查询都会被放行，绝大多数无效查询不会被放行

### 缓存击穿

#### 场景

对于一些设置了过期时间的key，如果这些key可能会在某些时间点被超高并发地访问，是一种非常“热点”的数据。在某个时间点过期的时候，恰好在这个时间点对这个Key有大量的并发请求过来，这些请求发现缓存过期一般都会从后端DB加载数据并回设到缓存，这个时候大并发的请求可能会瞬间把后端DB压垮

#### 解决方案

1. 使用分布式互斥锁，这种方式只允许一个线程来重建缓存，先将其他请求阻塞，等到缓存重建完毕，再重新去缓存中查询。这种方法比较简单，但是可能会存在死锁和线程池阻塞的风险

2. 不再设置过期时间，也就是说缓存中的数据不会"到期就失效"，同时设置一个"逻辑过期时间"，过了这个"逻辑过期时间"启动一个线程去数据库中查询最新的内容，然后更新缓存中的值。这样从根本上杜绝了热点数据失效的问题，但是唯一不足的就是，数据的一致性无法保证

   > 逻辑过期时间：
   >
   > 1. 从redis上看，确实没有设置过期时间，这就保证了不会出现热点key过期问题，也就是“物理”不过期
   > 2. 把过期时间存在key对应的value里，如果发现要过期了，通过一个后台的异步线程进行缓存的构建，也就是“逻辑”过期
