...
> 4. 两种操作拼接字符
>
> ![image-20211116110452405](imgs\左神算法课\75.png)

思路：质数只能进行第一种操作；合数由质数组成

所以只需要考虑操作一就可以

每个质数操作次数为`数字-1`，合数就是若干个质数的乘积

#### 5. 接雨水

> 5. 接雨水
>
> ![image-20211119155515398](imgs\左神算法课\77.png)
>
> [力扣链接](https://leetcode-cn.com/problems/trapping-rain-water/)

思路：只考虑当前的位置上有多少雨水，Min{Max(i左边最高点), Max(i右边最高点)}-arr[i]
...
优化：双指针，每次确定了左右max中较小的那边，就可以结算了，省空间
...
public int trap(int[] height) {
    ...
    int sum = 0;
    while (L <= R){
        // L指针处的可以结算了，因为可以确定瓶颈就是leftMax了
        int temp;
        if (rightMax > leftMax){
            temp = leftMax - height[L];
            sum += Math.max(temp, 0);
            leftMax = Math.max(leftMax, height[L++]);
        }else {
            temp = rightMax - height[R];
            sum += Math.max(temp, 0);
...
> 2. 转双向链表
>
> ![image-20211124103512015](imgs\左神算法课\83.png)
>
...
> 1. 数组topK
>
> ![image-20211116103103502](imgs\左神算法课\74.png)
>
> [力扣链接](https://leetcode-cn.com/problems/top-k-frequent-elements/)

思路：将数字和对应词频放入map，再根据词频将entry放入大根堆或固定容量的小根堆（如果大于peek()，则弹出最小的，推入大的这个数，需要自己定义固定容量的堆）
...
> 2. 字符流的topK
>
> 一串字符流，实时统计词频，并确定topK

思路：

1. 一个词频map
2. 自定义一个固定容量的小根堆，堆顶作为门槛来确定新的数据能否入堆
3. 自定义一个堆位置的map，不能够入堆的，位置记为-1

```java
class RealTimeTopK<T>{
    static class Freq<T>{
        T o;
        Integer freq;
        public Freq(T o, Integer freq) {
            this.o = o;
            this.freq = freq;
        }
        @Override
        public boolean equals(Object o1) {
            ...
            Freq<?> freq = (Freq<?>) o1;
            return Objects.equals(o, freq.o);
        }
        @Override
        public int hashCode() {
    ...
    private int size;
    final private int k;
    final private Freq<T>[] elements;
    final private HashMap<Freq<T>, Integer> posMap = new HashMap<>();
    // 将数据封装成Freq类
    final private HashMap<T, Freq<T>> t2freqMap = new HashMap<>();
    public RealTimeTopK(int k) {
        this.k = k;
        this.elements = new Freq[k];
    }
    public void offer(T o) {
        Freq<T> freq;
        if (!t2freqMap.containsKey(o)) {
            t2freqMap.put(o, (freq = new Freq<>(o, 0)));
            posMap.put(freq, -1);
        }
        freq = t2freqMap.get(o);
        freq.freq++;
        int pos = posMap.get(freq);
        // 不在堆中
        if (pos == -1) {
            // 满，判断放不放；未满，直接放
            if (size >= k) {
                if (elements[0].freq < freq.freq) {
                    posMap.put(pop(), -1);
                    elements[size] = freq;
                    lift(size);
                    ++size;
    ...
    private Freq<T> pop(){
        ...
        swap(0, size );
        sink(0);
        Freq<T> top = elements[size];
        elements[size] = null;
        posFresh();
        return top;
    }
    public T[] topK(){
        T[] topK = (T[]) new Object[k];
    ...
    private void lift(int index){
        while (index > 0 && elements[index - 1 >> 1].freq > elements[index].freq){
    ...
    private void sink(int index){
        while (index < size){
            ...
            int smaller = (rightChild >= size || elements[leftChild].freq < elements[rightChild].freq) ? leftChild : rightChild;
            if(elements[smaller].freq > elements[index].freq) return;
            swap(index, smaller);
            index = smaller;
        }
    }
    ...
    private void swap(int i, int j){
        Freq<T> temp = elements[i];
        elements[i] = elements[j];
        elements[j] = temp;
    }
    private void posFresh(){
        // 更新posMap
        for (int i = 0; i < elements.length; i++){
            posMap.put(elements[i], i);
        }
    }
}
```
...
> 1. 矩阵路径和
>
> ![image-20211119112111445](imgs\左神算法课\76.png)

1. 常规dp

```java
public int findSmallestSum(int[][] matrix){
    ...
    for (int i = matrix.length - 1; i >= 0; i--){
        for (int j = matrix[0].length - 1; j >= 0; j--){
            if (i == matrix.length - 1 && j == matrix[0].length - 1){
                ...
                dp[i][j] = matrix[i][j] + Math.min(i == matrix.length - 1 ? 
                                                   Integer.MAX_VALUE : dp[i + 1][j], 
                                                   j == matrix[0].length - 1 ? 
                                                   Integer.MAX_VALUE : dp[i][j + 1]);
            }
        }
    }
    return dp[0][0];
...
我们要的是第一行的dp\[0][0]，有没有办法，从最后一行开始，不断计算得到第一行。答案是确定的
    ...
    for (int i = matrix.length - 1; i >= 0; i--){
        ...
        for (int j = matrix[0].length - 1; j >= 0; j--){
            if (i == matrix.length - 1 && j == matrix[0].length - 1){
                ...
                dp[j] = matrix[i][j] +
                        Math.min(j == matrix[0].length - 1 ? 
                                 Integer.MAX_VALUE : dp[j + 1],
                                i == matrix.length - 1 ? 
                                 Integer.MAX_VALUE : dp[j]);
            }
        }
    }
    return dp[0];
...
> 技巧：逆序更新，临时变量
>
> 宽度（每一行的列数）很大但长度不大，就以列为单位更新；长度很大但宽度不大，就以行为单位更新
...
> 1. 旋转词
>
> ![image-20211122093956872](imgs\左神算法课\78.png)

思路：生成两倍的原字符串，如：12345，则生成：1234512345，在生成的字符串中任意截取5个，都为旋转词。因此，只需要判断是否为子串即可

```java
public boolean isRotatedWord(String a, String b){
    String aa = a + a;
    return indexOf(aa, b) != -1;
}
public int indexOf(String father, String son){
    if (son.length() == 0) return -1;
    int[] arr = generateArr(son);
    int fl = father.length(), sl = son.length();
    int fIndex = 0, sonIndex = 0;
    char[] fch = father.toCharArray(), sch = son.toCharArray();
    while (fIndex < fl && sonIndex < sl){
        if (fch[fIndex] == sch[sonIndex]){
            ++fIndex;
            ++sonIndex;
...
public int[] generateArr(String son){
    int[] arr = new int[son.length()];
    arr[0] = -1;
    for (int i = 2; i < son.length(); i++){
        char ch = son.charAt(i - 1);
        int pre = arr[i - 1];
        while (pre != - 1 && ch != son.charAt(pre)){
            pre = arr[pre];
        }
        arr[i] = pre + 1;
    }
...
> 1. 达标字符
>
> ![image-20211123105025189](imgs\左神算法课\79.png)

1. 打表法，发现是斐波那契数列

0、1→1；10、11→2；101、110、111→3；1110、1101、1011、1010、1111→5

2. 贪心，首位必须是1，则考虑F(N-1)；第二位新增为0的可能性，即考虑F(N-2)

```java
public int legal(int n){
...
> 2. 捡木棍
>
> ![image-20211123111805691](imgs\左神算法课\81.png)

最接近n的斐波那契数（斐波那契数列中任一数都无法构成三角形）
...
> 1. 非严格背包
>
> ![image-20211123111652779](imgs\左神算法课\80.png)

严格背包：对于前i个数，dp\[i][j]表示前i个数组成j的组合数

```java
dp[i][j] = dp[i - 1][j]; // 不考虑第i个
dp[i][j] += dp[i - 1][j - v[i]]; // 考虑第i个
```

取最后一行所有j <= w的，即为答案

```java
public int _01bag(int[] v, int w){
...
> 1. 匹配工作
>
> ![image-20211124094452888](imgs\左神算法课\82.png)
...
```java
public int[] findJob(Job[] jobs, int[] ability){
    Comparator<Job> jobComparator = (o1, o2) -> {
        int hardDiff = o1.hard - o2.hard;
        int moneyDiff = o2.money - o1.money;
        return hardDiff == 0 ? moneyDiff : hardDiff;
    };
    ...
    Job preJob = jobs[0];
    for (Job job: jobs){
        if (job.hard == preJob.hard) continue;
        if (job.money < preJob.money) continue;
        list.add(job);
        preJob = job;
    }
    ...
    for (int i = 0; i < ability.length; i++){
        ...
        for (int j = list.size() - 1; j >= 0; j--){
            if (a >= list.get(j).hard){
                salary[i] = list.get(j).money;
                break;
            }
        }
    }
